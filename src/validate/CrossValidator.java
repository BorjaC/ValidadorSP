 ...
 */
public class CrossValidator extends Validator {
	...
	private int mode = 1; /* Marca el modo con el que se realiza la validacion */
	/* modo = 1 usando el mejor modelo */
	/* modo = 2 usando la media de los modelos */
	 ...
	 */
	public CrossValidator(int n, boolean save) {
		super();
		this.n = n;
		this.save = save;
	}
	 ...
	 */
	@Deprecated
	public CrossValidator(String treebank, int n){
		super(treebank);
		this.n = n;
	}
	 ...
	 * @param save parametro que indica que se quiere guardar el modelo
	 */
	public CrossValidator(String treebank, int n, boolean save){
		super(treebank);
		this.n = n;
		this.save = save;
	}
	 ...
	 * 
	 * @param setLenght
	 *            tamaño del conjunto de cada particion
	 */
		...
		try {
			...
			for (int i = 0; i < n; i++) {
				...
				for (int j = 0; j < tamPart; j++, idx++) {
					rows.write(trees.get(indices.get(idx)).getSentence() + "\n");
	...
	public ResultsModel validate(int setLenght) {
		...
		ResultsModel d = new ResultsModel();
		...
		for (int i = 0; i < n; i++) {
			...
			/* Obtener rendimiento */
			d = TreeVerificator.validate(out, inputFile.get(i));
		...
		/* Guardar el mejor modelo, en caso de que se requiera */
		if(save && mode == 1){
			moveFlie(bestTrain);
			moveFlie(bestModel);
			moveFlie(bestOutput);
	 ...
	 */
	public List<Tree> getTreesError(int setLenght) {
		List<Tree> treesError = new ArrayList<Tree>();
		...
		try {
			golden0_5 = new BufferedWriter(new FileWriter("golden0_5.txt"));
			golden0_6 = new BufferedWriter(new FileWriter("golden0_6.txt"));
			golden0_7 = new BufferedWriter(new FileWriter("golden0_7.txt"));
			golden0_8 = new BufferedWriter(new FileWriter("golden0_8.txt"));
			golden0_9 = new BufferedWriter(new FileWriter("golden0_9.txt"));
			golden1 = new BufferedWriter(new FileWriter("golden1.txt"));

			out0_5 = new BufferedWriter(new FileWriter("out0_5.txt"));
			out0_6 = new BufferedWriter(new FileWriter("out0_6.txt"));
			out0_7 = new BufferedWriter(new FileWriter("out0_7.txt"));
			out0_8 = new BufferedWriter(new FileWriter("out0_8.txt"));
			out0_9 = new BufferedWriter(new FileWriter("out0_9.txt"));
			out1 = new BufferedWriter(new FileWriter("out1.txt"));
			...
			for (int i = 0; i < numSentences; i++) {
				...
				/* Obtener rendimiento */
				ResultsModel d = TreeVerificator.validate(out, inputFile.get(i));
